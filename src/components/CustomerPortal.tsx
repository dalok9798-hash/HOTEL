...
import { 
  ...
  Award, Star, ThumbsUp, Plus, Minus, Trash2, X, ChefHat, Check, Heart, MoreVertical, ShieldAlert
} from "lucide-react";
...
interface CustomerPortalProps {
  ...
  currentTab: "customer" | "staff" | "kitchen" | "admin";
  setCurrentTab: (tab: "customer" | "staff" | "kitchen" | "admin") => void;
  session: { role: string; username: string } | null;
  onLogout: () => void;
}
          ...
          <div className="flex items-center gap-1.5 relative">
            ...
            <div className="relative">
              <button
                ...
                title="Management Portals & Login"
              >
              ...
              {isStaffDropdownOpen && (
                <>
                  ...
                  <div className="absolute right-0 mt-2 w-56 bg-slate-950 text-slate-100 rounded-2xl p-2.5 shadow-2xl border border-slate-800 z-50 animate-fade-in-down font-sans text-xs">
                    <div className="px-2 py-1.5 border-b border-slate-800 mb-1.5 flex items-center justify-between">
                      <span className="text-[10px] text-slate-400 uppercase tracking-widest font-black">Authorized Portals</span>
                      <ShieldAlert className="w-3.5 h-3.5 text-emerald-450 animate-pulse" />
                    </div>

                    {session ? (
                      <div className="space-y-1">
                        <div className="px-2 py-1.5 bg-slate-900 border border-slate-800 rounded-xl text-[10px] text-emerald-400 font-bold mb-2">
                          <span className="block opacity-70">CURRENTLY SIGNED IN AS:</span>
                          <span className="block text-white mt-0.5 truncate">👤 {session.username} ({session.role.toUpperCase()})</span>
                        </div>
                        ...
                        <button
                          ...
                          className="w-full text-left font-bold text-slate-300 hover:text-white rounded-xl px-2.5 py-2 hover:bg-slate-900 flex items-center gap-1.5 transition-colors cursor-pointer"
                        >
                        ...
                        <button
                          ...
                          className="w-full text-left font-bold text-slate-300 hover:text-white rounded-xl px-2.5 py-2 hover:bg-slate-900 flex items-center gap-1.5 transition-colors cursor-pointer"
                        >
                          ⚙️ Switch to Admin Tools
                        </button>

                        <div className="border-t border-slate-850 mt-1.5 pt-1.5">
                          <button
                            onClick={() => {
                              setIsStaffDropdownOpen(false);
                              onLogout();
                            }}
                            className="w-full text-center font-bold text-red-450 hover:bg-red-500/10 hover:text-red-400 border border-red-500/10 rounded-xl py-2 flex items-center justify-center gap-1.5 transition-all cursor-pointer"
                          >
